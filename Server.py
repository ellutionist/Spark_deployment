...
class Server:
    ...
    def upload_profile(self):
        ...
        if not self._check_profile():
            ...
            pwd = self.__conn.run("pwd").stdout.strip()
            self.__conn.put(profile_path, '{pwd}/profile'.format(pwd=pwd))
            self.__conn.run('sudo chown root ~/profile', pty=True, watchers=[self.__sudopass])
            self.__conn.run('sudo chmod 644 ~/profile', pty=True, watchers=[self.__sudopass])
            self.__conn.run('sudo mv ~/profile /etc/profile', pty=True, watchers=[self.__sudopass])
    ...
    def install_java(self):
        ...
        if not self._check_java():
            ...
            self.__conn.run('sudo mv ~/Downloads/{folder_name} {JAVA_HOME}'.
                            format(folder_name=java_folder_name, JAVA_HOME=JAVA_HOME),
                            pty=True, watchers=[self.__sudopass])
    ...
    def install_spark(self):
        ...
        if not self._check_spark():
            ...
            self.__conn.run('sudo mv ~/Downloads/{folder_name} {SPARK_HOME}'.
                            format(folder_name=spark_folder_name, SPARK_HOME=SPARK_HOME),
                            pty=True, watchers=[self.__sudopass])
    ...
    def _connect(self) -> Connection:
        """
        Connect to the server using ssh. Try to log in with public key at first, if failed then set up public key
        authentication.
        ...
        try:  # try to connect with private key first
            conn = Connection(host="localhost", port=self.__port, user=self.__username,
                              config={"key_filename": private_key_path})
            conn.run("pwd")
        except SSHException:  # if failed, connect with password, then set private key authentication.
            conn = Connection(host="localhost", port=self.__port, user=self.__username,
    ...
    def _set_ssh_authentication(self, conn: Connection):
        ...
        pwd = conn.run("pwd").stdout.strip()

        try:
            conn.run("cd ~/.ssh")  # check the existence of ~/.ssh/
    ...
    def _check_java(self) -> bool:
        ...
        JAVA_HOME = self.__config["java"].get("JAVA_HOME")
        try:
            self.__conn.run("cd {JAVA_HOME}".format(JAVA_HOME=JAVA_HOME))  # check the directory of java
    ...
    def _check_spark(self) -> bool:
        ...
        SPARK_HOME = self.__config["spark"].get("SPARK_HOME")
        try:
            self.__conn.run("cd {SPARK_HOME}".format(SPARK_HOME=SPARK_HOME))  # check the directory of spark
    ...
    def _check_profile(self) -> bool:
        ...
        result_java: Result = self.__conn.run("source /etc/profile && echo $JAVA_HOME")
        result_spark: Result = self.__conn.run("source /etc/profile && echo $SPARK_HOME")

        return result_java.stdout.strip() == JAVA_HOME and result_spark.stdout.strip() == SPARK_HOME

    def _ensure_directory(self, path: str):
        """
        Universal method to check whether a director exists and if not, create it.
        :param path: the specific path
        :return: None
        """
        try:
            self.__conn.run("cd {path}".format(path=path))
        except UnexpectedExit:
...
if __name__ == '__main__':
    raise SparkInstallationFailure(10001)